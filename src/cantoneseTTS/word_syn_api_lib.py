...
# # Default paths
# parser.add_argument('--canPhones', default="./jyutping-wong-44100-v9/jyutping-wong/",
#                     help="Folder containing Cantonese wavs")
...
# parser.add_argument('--language', "-l", action="store", dest="language", type=str,
#                     help="Choose the language for output", default=None)
...
# parser.add_argument('--crossfade', '-c', action="store_true", default=True,
#                     help="Enable slightly smoother concatenation by cross-fading between tokens")
# parser.add_argument('--volume', '-v', default=None, type=int,
#                     help="An int between 0 and 100 representing the desired volume")
# parser.add_argument('--speed', '-s', default=None, type=float,
#                     help="A float between 0 - 3 representing the desired speed")

class libConfig:
    canPhones ="./jyutping-wong-44100-v9/jyutping-wong/"
    mandPhones ="./pinyin-yali-44100/"
    text= "1/01/1991，32。翻译都要执行多个翻译系统，这带来巨大的计算成本。如今，许多领域都正在被神经网路技术颠覆。测试一"
    phrase = [text,]  # 以后可以放多个
    language= "c"
    # play=
    crossfade=True
...
#     exit()  # exit(0) 则表示程序是正常退出的 exit(1) 表示发生了错误
...
def assign_paths(language):
    ...
    if language == None:
        language = check_lang(libArgs.phrase[0])
        # language = check_lang(args.phrase)
        libArgs.language = language
        # Cantonese
    if language == "c":
        path = libArgs.canPhones
        dictpath = './wordsyn/phonedict_dict_can'
    # Mandarin
    elif language == "p":
        path = libArgs.mandPhones
        # path = mandPhones
        dictpath = "./wordsyn/phonedict_dict_pth_perc"
...
path, dictpath = assign_paths(libArgs.language)  # 选择语言
...
def main():
    ...
    inputseq = libArgs.phrase[0]
    # Step 2 - Put the text in a Sequence instance
    inputseq = Sequence(inputseq)
    # print("inputse说明:", inputseq)
    # print("inputseq.tokens说明:", inputseq.tokens)
    ...
    output = adjust_volume(volume=libArgs.volume, object=output)
    # output = adjust_volume(volume=80, object=output)
    # Todo:volume直接写成要传递的值


    # Step 6 - Save it to the target file (if the user use -o <args.outfile>)
    save(output_file=libArgs.outfile, object=output)
    # save(output_file="../outfile_test/", object=output)
